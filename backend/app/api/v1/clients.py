...
async def list_clients(
    ...
    if search:
        query = query.where(Client.last_name.ilike(f"%{search}%") | Client.phone.ilike(f"%{search}%"))
...
@router.post(
    "/", response_model=ClientResponse, status_code=201, dependencies=[Depends(require_permission("clients", "write"))]
)
...
@router.get(
    "/{client_id}", response_model=ClientResponse, dependencies=[Depends(require_permission("clients", "read"))]
)
