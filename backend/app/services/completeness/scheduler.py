...
class CompletenessReminderScheduler:
    ...
    async def check_and_send_reminders(self) -> int:
        ...
        async with self._session_factory() as session:
            ...
            for case_data in cases_with_missing:
                ...
                if already_sent:
                    self._logger.debug("Reminder already sent for case %s on day %s", case_id, target_day)
                    continue
                ...
                try:
                    ...
                    self._logger.error("Failed to send reminder for case %s: %s", case_id, e, exc_info=True)
    ...
    async def _get_cases_with_missing_items(self, session: AsyncSession) -> list[dict]:
        ...
        subq = (
            ...
            .where(CaseChecklistItem.status.in_([ChecklistItemStatus.MISSING, ChecklistItemStatus.REJECTED]))
            .group_by(CaseChecklistItem.case_id)
            .having(func.count(CaseChecklistItem.id) > 0)
            .subquery()
        )

        stmt = select(subq.c.case_id, subq.c.init_date, subq.c.item_ids)
        ...
        for row in rows:
            ...
            for item_id in item_ids:
                ...
                try:
                    ...
                    self._logger.warning("Failed to get item name for %s: %s", item_id, e)
                    missing_items.append(item_id)
    ...
    async def _was_reminder_sent(
        ...
        stmt = select(func.count(Notification.id)).where(
            and_(
                ...
                Notification.title.like("%Напоминание: нужны документы%"),
