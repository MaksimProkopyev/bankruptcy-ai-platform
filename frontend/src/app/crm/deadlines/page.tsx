      ...
      <div className="flex gap-2 mb-6">
        {(["upcoming", "overdue", "all"] as const).map((f) => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-3 py-1.5 text-sm rounded-lg border ${filter === f ? "bg-brand-50 border-brand-200 text-brand-700 font-medium" : "bg-white border-gray-200 text-gray-600"}`}>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      <Link href={`/cases/${d.case_id}`} className="text-xs text-brand-600 hover:underline">
