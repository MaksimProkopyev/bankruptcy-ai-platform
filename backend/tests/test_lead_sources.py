...
@pytest.mark.asyncio
async def test_run_collector_and_list_leads(client: AsyncClient):
    run = await client.post("/api/v1/lead-sources/fssp/run")
    ...
    leads_resp = await client.get("/api/v1/lead-sources/fssp/leads")
    assert leads_resp.status_code == 200
    leads = leads_resp.json()
    assert len(leads) >= 1
    assert leads[0]["source"] == "fssp"

    stats_resp = await client.get("/api/v1/lead-sources/stats")
...
@pytest.mark.asyncio
async def test_convert_lead_to_client_and_case(client: AsyncClient):
    await client.post("/api/v1/lead-sources/fssp/run")
