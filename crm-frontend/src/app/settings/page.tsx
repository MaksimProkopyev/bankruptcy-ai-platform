      ...
      <div className="flex gap-1 mb-6 border-b border-gray-200">
        {tabs.map((t) => (
          <button key={t.key} onClick={() => setActiveTab(t.key)}
            className={`px-4 py-2.5 text-sm border-b-2 ${activeTab === t.key ? "border-primary text-primary font-medium" : "border-transparent text-gray-500"}`}>
            {t.label}
          </button>
        ))}
      </div>
          ...
          {user ? (
            <div className="space-y-4">
              ...
              <div>
                ...
                <span className="inline-block px-3 py-1.5 bg-primary-light text-primary-dark rounded-lg text-sm font-medium">
      ...
      {activeTab === "team" && (
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <button className="px-4 py-2 text-sm bg-accent text-text-on-dark rounded-lg">+ Добавить сотрудника</button>
          </div>
