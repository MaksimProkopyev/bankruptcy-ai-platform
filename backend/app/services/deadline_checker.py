...
from app.models.models import CaseEvent, Deadline, Notification
...
async def check_deadlines(db: AsyncSession) -> dict:
    ...
    # 1. Mark overdue
    result = await db.execute(select(Deadline).where(and_(Deadline.status == "pending", Deadline.due_date < now)))
