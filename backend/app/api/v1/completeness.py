...
async def _verify_case_access(
    ...
    if user.role == "client":
        ...
        if not client or case.client_id != client.id:
            raise HTTPException(
                ...
                detail="Access denied: case does not belong to this client"
            )
        return
    ...
    raise HTTPException(
        status_code=status.HTTP_403_FORBIDDEN,
        detail="Insufficient permissions to access this case"
    )
...
async def get_completeness(
    ...
    if not progress:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Checklist not initialized for this case"
        )
...
async def update_item(
    ...
    if current_user.role == "client" and update.status not in (
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Clients can only upload documents or mark them as missing"
        )
    ...
    checker = CompletenessChecker(session)
    try:
        return await checker.update_item(
            case_id, item_id, update, reviewer_id=reviewer_id
        )
    except ValueError as e:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
